	...
	<div class="item">
		<?php if( ( $url = $service->getMediaUrl() ) != '' ) : ?>
			<div class="item-icons">
				<img src="<?= $enc->attr( $this->content( $url ) ); ?>"  width="80" height="auto" />
			</div>
		<?php endif; ?>
			...
			<?php foreach( $attributes as $attribute ) : ?>
				<?php if( $attribute->getType() === $type ) : ?>
					...
					<li class="<?= $enc->attr( $type . '-' . $attribute->getCode() ); ?>">
						...
						<?php break; default: ?>
							<?php switch( $attribute->getCode() ) : case 'freyversand.datum': ?>
								<span class="value"><?= $enc->html( date_create( $attribute->getValue() )->format( $this->translate( 'client', 'Y-m-d' ) ) ); ?></span>
							<?php break; default: ?>
