...
$alcohol = false;
foreach( $this->summaryBasket->getProducts() as $orderProduct ) {
	if( $orderProduct->getAttribute( 'alcohol', 'hidden' ) !== null ) {
		$alcohol = true;
	}
}
...
<?php if( $alcohol ) : ?>
	<div class="checkout-standard-summary-option-alcohol col-sm-12">
		<h3><?= $enc->html( $this->translate( 'client', 'Jugendschutz beim Alkoholverkauf' ), $enc::TRUST ); ?></h3>

		<div class="single <?= ( isset( $errors['option']['alcohol'] ) ? 'error' : '' ); ?>">
			<input type="hidden" name="<?= $enc->attr( $this->formparam( array( 'cs_option_alcohol' ) ) ); ?>" value="1" />
			<input id="option-alcohol" type="checkbox" value="1"
				name="<?= $enc->attr( $this->formparam( array( 'cs_option_alcohol_value' ) ) ); ?>"
				<?= ( $this->param( 'cs_option_alcohol_value', null ) == 1 ? 'checked="checked"' : '' ); ?>
			/>
			<p>
				<label for="option-alcohol">
					<?= $enc->html( $this->translate( 'client', 'Das Gesetz verbietet den Verkauf von Wein, Bier und Apfelwein an unter 16-Jährige. <br>
Ich bestätige hiermit, dass ich selber mindestens 16 Jahre alt bin und auch die Empfängerin bzw. der Empfänger nicht zu den unter 16-Jährigen gehören.<br>' ), $enc::TRUST ); ?>
				</label>
			</p>
		</div>
	</div>
