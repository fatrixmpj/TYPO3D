...
<ul class="selection">
	<?php foreach( $this->get( 'attributeCustomItems', [] ) as $id => $attribute ) : ?>
		<li class="select-item <?= $enc->attr( $attribute->getCode() ); ?>">
			<div class="select-name"><?= $enc->html( $attribute->getTypeName() ); ?></div>
...
<ul class="selection">
	<?php foreach( $attributeConfigItems as $code => $attributes ) : ?>
		...
		<li class="select-item <?= $enc->attr( $layout ) . ' ' . $enc->attr( $code ); ?>">
			<div class="select-name"><?= $enc->html( reset( $attributes )->getTypeName() ); ?></div>
				...
				<?php elseif( $layout === 'image' ) : ?>
					...
					<ul class="image-list">
						<?php foreach( $attributes as $attrId => $attribute ) : ?>
							...
							<li class="form-inline image-entry">
								<input type="radio" value="<?= $enc->attr( $attrId ); ?>"
									name="<?= $enc->attr( $this->formparam( array( 'b_prod', 0, 'attrconfid', 'id', '' ) ) ); ?>"
								/>
								<label class="form-control image-label" for="option-<?= $enc->attr( $attrId ); ?>">
									
									<input type="hidden" value="1" name="<?= $enc->attr( $this->formparam(array( 'b_prod', 0, 'attrconfid', 'qty', '' ) ) ); ?>" />
									...
									<?php foreach( $attribute->getRefItems( 'price', 'default', 'default' ) as $priceItem ) : ?>
										<?= $enc->html( sprintf(
											$this->translate( 'client', '%1$s %2$s' ),
											$this->number( $priceItem->getValue() + $priceItem->getCosts() ),
											$this->translate( 'currency', $priceItem->getCurrencyId() )
										), $enc::TRUST ); ?>
									<?php endforeach ?>
